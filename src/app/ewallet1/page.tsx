...
export default function Home() {
  ...
  const [tunaiCounter, setCounter] = useState<number>(100);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      setCounter(
        localStorage.getItem("ewallet1")
          ? parseInt(localStorage.getItem("ewallet1")!)
          : 100
      );
      setParticipants(
  ...
  const startRandomizer = () => {
    if (!audio || !confettiAudio || participants.length === 0) return;
        ...
        const slowInterval = setInterval(() => {
          ...
          if (counter >= totalIterations) {
            ...
            if (participants.length > 0) {
              ...
              updatedParticipants.splice(choosenIndex, 1);
    ...
    localStorage.setItem("ewallet1", (tunaiCounter - 1).toString());
  };
    ...
    <>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#FAEFD9]">
        <h1 className="text-3xl font-bold mb-6 text-[#401606]">
          Undian 10 E-Wallet 250 Ribu Rupiah
        </h1>
        <div className="mb-6 relative">
          <img src="/250k.png" alt="Kaos Running" className="w-48 h-48" />
        </div>
        {!isAnimating ? (
