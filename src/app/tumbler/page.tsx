...
export default function Home() {
  ...
  const [winners, setWinners]: any = useState(
    JSON.parse(localStorage.getItem("winnersTumbler") || "[]")
  );
    ...
    if (typeof window !== "undefined") {
      setCounter(
        localStorage.getItem("tumbler")
          ? parseInt(localStorage.getItem("tumbler")!)
          : 100
      );
      setParticipants(
          ...
          const slowInterval = setInterval(() => {
            ...
            if (counter >= totalIterations) {
              ...
              if (winners.length < totalWinners && participants.length > 0) {
                ...
                localStorage.setItem("winnersTumbler", JSON.stringify(winners));
              }
            }
          }, intervalTime * 2);
        }
    ...
    <>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#FAEFD9]">
        <h1 className="text-4xl font-bold mt-28 mb-4 text-[#401606]">
          Undian 3 Buah Tumbler
        </h1>
        <div className="mb-6 relative">
          <img src="/corkcicle.png" alt="Tumbler" className="w-60 h-60" />
        </div>
        {!isAnimating ? (
